...
conditions, and [custom mods](dev/engine-mods) can add any [`Filter`](spec/filtering.md#1-the-filter-value-object)
to the list.
...
- `flare_enclosure(model|entry_row, string field = "enclosure")` for retreiving file enclosure data as you would with
  ...
  ```
- `flare_project(list_spec, context)` for running a projection directly from a template: returns the view
  for the given `ListSpec` and context configuration, without going through a content element's engine.
