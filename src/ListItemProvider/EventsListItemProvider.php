...
class EventsListItemProvider
{
    public function __construct(
        private readonly ListQueryDirector $queryDirector,
    ) {}
    ...
    protected function fetchEntriesGrouped(
        ...
        $options = new ListQueryConfig(
            list: $listDefinition,
            context: $contextConfig,
            filterValues: [], // cannot work this way, must be collected in projector anyway
            attributes: ['override_select' => $reduceSelect ? ['id', 'startTime', 'endTime', 'repeatEach', 'repeatEnd'] : null]
        );

        $qb = $this->queryDirector->createQueryBuilder($options);
